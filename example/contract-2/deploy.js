...
const installBundle = async (pathResolve, zoe, board) => {
  ...
  const moolaIssuer = await E(board).getValue(
    minterConstants.TOKEN_ISSUER_BOARD_ID,
  );
  ...
  const NFT_BRAND_BOARD_ID = await E(board).getId(
    await E(nftIssuer).getBrand(),
  );
...
const deployContract = async (homePromise, { pathResolve }) => {
  ...
  const defaultsFile = pathResolve(`../react-ui/src/nftMinterConstants.js`);
...
`;
  ...
  // to test TypeScript App
  await fs.promises.writeFile(
    `../react-ui-ts/src/moolaMinterConstants.mjs`,
    defaultsContents,
  );
};
